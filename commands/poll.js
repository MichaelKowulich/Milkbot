...
module.exports = {
    ...
    async execute(message, args, bot){
        ...
        .setColor('	#3CB371')
        .setTitle("**".concat(options[0].concat("**").replace(/,/g," ").replace(/\"/g, "")));
        ...
        for (i=1; i<options.length; i++) {
            ...
            {name: PollNumber, value: options[i].replace(/,/g, " ").replace(/\"/g, "")}
            );
        }
        ...
        message.channel.messages.fetch(messageID).then (messageReaction => {
            for(i=1; i<options.length; i++) {
                if (i == 1) {messageReaction.react("1️⃣");}
                if (i == 2) {messageReaction.react("2️⃣");}
                if (i == 3) {messageReaction.react("3️⃣");}
                if (i == 4) {messageReaction.react("4️⃣");}
                if (i == 5) {messageReaction.react("5️⃣");}
                if (i == 6) {messageReaction.react("6️⃣");}
                if (i == 7) {messageReaction.react("7️⃣");}
                if (i == 8) {messageReaction.react("8️⃣");}
                if (i == 9) {messageReaction.react("9️⃣");}
                if (i == 10){ messageReaction.react("🔟");}
            }
        });
        ...
        .setColor('#3CB371')
        .setTitle('Please Format >Poll Command as follows (not case sensitive):')
        .addFields (
            {name: '>poll "Question to Poll" "Opt 1" "Opt 2" "Opt3" ... "Opt 10" ', value: 'Note: Questions and options must be between double quotes. The most options MilkBot will allow is 10 and each poll must have at least 1 option.'}
        );
    ...
    function pollResults (messageOBJ, opts) {
        ...
        for (i=0; i<categorical.length-1; i++) {
            cmd = cmd.concat('"').concat(categorical[i+1]).concat('" ');
